...
def split_train_test_valid(df, target: str, test_size=0.3, val_size=0.3, verbose=False, **kwargs):
    ...
    X_train, X_val, y_train, y_val = train_test_split(X_data, y_data, test_size=val_size, **kwargs)
    if verbose:
...
def get_corr_matrices(data, method="pearson"):
    ...
    while row <= n:
        ...
        row += 1
    pbar.close()
    return data.corr(method=method), pd.DataFrame(cramers_mat, index=data.columns, columns=data.columns)


def select_feats_corr(data, corr_matrices=None, sens_lin=0.7, sens_cramer=0.4, method="pearson"):
    ...
    for x_i in data.columns:
        ...
        for x_j in feat_list:
            if abs(corr_lin.loc[x_i, x_j]) > sens_lin or corr_cramer.loc[x_i, x_j] > sens_cramer:
                u = False
        if u:
...
def plot_hier_corr(corr_matrix):
    ...
    dendro = hierarchy.dendrogram(corr_linkage, labels=corr_matrix.columns, ax=ax1, leaf_rotation=90)
...
def select_features_corr(df, corr_matrices: tuple, pearson_sens=0.8, cramer_sens=0.8, verbose=False):
    ...
    if verbose:
        print("Selected {} predictors".format(len(DF_predictors[DF_predictors["reason"] == "added"])))

    return DF_predictors


def select_feats(
    X_train, y_train, gini_and_iv_stats, pearson_corr, cramer_corr, pearson_sens=0.8, cramer_sens=0.8, random_seed=42
):
    np.random.seed(random_seed)
    ...
    gini_iv_subset = gini_and_iv_stats[gini_and_iv_stats["predictor"].isin(X_train.columns)]
    # Отбор фичей по корреляциям, Gini и IV
    corr_select_res = select_features_corr(
        gini_iv_subset, (pearson_corr, cramer_corr), pearson_sens=pearson_sens, cramer_sens=pearson_sens
    )
    # Исключение предикторов с положительными коэффициентами
    feat_list = corr_select_res.loc[corr_select_res["reason"] == "added", "predictor"].to_list()
    feat_list = positive_coef_drop(X_train[feat_list], y_train, gini_data, iv_ordered_feats, seed=random_seed)
...
def positive_coef_drop(X, y, gini_data, iv_ordered_feats, seed=42, verbose=False, enable_tqdm=False):
    ...
    for _ in predictors:  # исключение предикторов с положительными коэфициентами
        ...
        grid_search = GridSearchCV(model_grid, param_grid_model, scoring="roc_auc", cv=skf)
        ...
        DF_model_inf = DF_model_inf.merge(
            gini_data[["predictor", "gini_train", "gini_test"]], how="left", on="predictor"
        ...
        DF_model_inf = DF_model_inf.merge(iv_ordered_feats, how="left", left_on="predictor", right_index=True)
        k_sum = (DF_model_inf["coef"] * DF_model_inf["IV"]).sum()

        DF_model_inf["coef_K"] = DF_model_inf["coef"] * DF_model_inf["IV"] / k_sum
        DF_model_inf_2 = DF_model_inf.loc[DF_model_inf["coef"] >= 0].sort_values("IV").reset_index(drop=True)
