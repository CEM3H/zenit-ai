...
def compare_series(s1, s2, ret_index=False):
    ...
    ret_index : bool,
        модификатор вывода результатов
    ...
    res = pd.DataFrame(
        {
            ...
            "many_floats_w_cat": np.hstack(
                (np.random.rand(5000) * 100, np.array(["d", "f"] * 2500))
            ),
            "many_floats_w_cat_and_nan": np.hstack(
                (np.random.rand(7000) * 100, np.array(["d", "f", np.nan] * 1000))
            ),
            "many_floats_w_outliers": np.hstack(
                (np.random.rand(5000) * 100, np.array([-10000, 10000] * 2500))
            ),
            ...
            "integers_w_small_cat": np.hstack(
                (np.array(np.random.choice(range(10), 9990), float), [100] * 10)
            ),
            "floats_w_na": np.hstack(
                (
                    np.array(np.random.choice(range(10), 9000), float),
                    np.full(1000, np.nan),
                )
            ),
            "integers_w_letters": np.hstack(
                (np.random.choice(range(10), 9000), np.array(["d", "f"] * 500))
            ),
            ...
            "letters_uneq_freq": np.random.choice(
                ["a", "b", "c"], 10000, p=[0.6, 0.3, 0.1]
            ),
            "letters_w_na": np.hstack(
                (
                    np.array(np.random.choice(["a", "b", "c"], 9000), object),
                    np.full(1000, np.nan),
                )
            ),
    ...
    res = pd.DataFrame(
        {
            "digits": np.random.choice(
                range(10),
                10000,
                p=[0.15, 0.15, 0.05, 0.05, 0.1, 0.1, 0.1, 0.01, 0.19, 0.1],
            ),  # изменены веса
            "integers": np.random.choice(range(100), 10000),
            "floats": np.random.choice(np.linspace(1, 20, 100), 10000),
            "large_floats": np.random.choice(np.linspace(1, 10 ** 6, 10 * 3), 10000),
            "integers_w_neg": np.random.choice(
                range(-100, 101), 10000
            ...
            "floats_w_na": np.hstack(
                (
                    np.array(np.random.choice(range(10), 8000), float),
                    np.full(2000, np.nan),
                )
            ...
            "letters": np.random.choice(["a", "b", "c", "d", "e", "f", "g"], 10000),
            "letters_uneq_freq": np.random.choice(
                ["a", "b", "c"], 10000, p=[0.1, 0.6, 0.3]
            ),  # изменены веса
            "letters_w_na": np.hstack(
                (
                    np.array(np.random.choice(["a", "b", "c", "d"], 9000), object),
                    np.full(1000, np.nan),
                )
            ),  # добавлена категория
